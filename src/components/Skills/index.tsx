...
export default function Skills() {
  ...
  return (
    <>
      <Section id="skills" title="Skills" isAlternate={true}>
            ...
            <div key={index}>
              ...
              <div className="flex flex-wrap gap-1">
                {skillData.skills.map((skill, index) => (
                  <Skill key={index} category={skillData.category}>
                    {skill}
                  </Skill>
                ))}
              </div>
          ...
          <div>
            ...
            <div className="grid gap-3 grid-cols-2 sm:grid-cols-3">
              {certifications.map((certification, index) => (
                <div
                  key={index}
                  className="border border-gray-200 bg-white p-4 rounded-sm hover:drop-shadow transition-[filter]"
                >
